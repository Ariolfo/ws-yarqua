...
/// <summary>
/// Catálogo de sensores desde YarqtbSensor / YarqtbRed.
/// </summary>
public sealed class SensorCatalogService : ISensorCatalogService
{
    ...
    public SensorCatalogService(YarquaDbContext db)
    {
        _db = db;
    }
    ...
    private IQueryable<YarqtbSensor> QueryActive() =>
        _db.Sensores.AsNoTracking()
            .Include(s => s.Red)!.ThenInclude(r => r!.Pais)
            .Include(s => s.Cultivo)
            .Where(s => s.SensActivo);

    private static PhysicalSensor ToPhysical(YarqtbSensor s) =>
        new(
            s.SensNombre,
    ...
    private static CatalogSensorDto ToDto(YarqtbSensor s) => new()
    {
        Id = s.SensId,
        Name = s.SensNombre,
        NetworkId = s.RedId,
